...
void		fnInitCommonRegExModule	( void )
{
  static CLASSINFO ClassInfoTable []	= {
    { (SHTYPETAG) eshRegExTag,
      SZPLOB,
      "regex",
      (sizeof(PLOBREGEX)-sizeof(PLOBHEADER)-
       sizeof(((LPPLOBREGEX)0)->timeRegEx)-
       sizeof(((LPPLOBREGEX)0)->regEx))*nBitsPerByte,
      (sizeof(((LPPLOBREGEX)0)->timeRegEx)+
       sizeof(((LPPLOBREGEX)0)->regEx))*nBitsPerByte,
      (LPCSTR) NULL,
  ...
  PROCEDURE	( fnInitCommonRegExModule );
...
{
  PROCEDURE	( fnDeinitCommonRegExModule );
