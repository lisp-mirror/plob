...
void			fnInitCommonLockModule		( void )
{
  static CLASSINFO	ClassInfoTable []	= {
    { (SHTYPETAG) eshSumLockTag,
        SZPLOB,
	"sum-lock",
	(sizeof(SUMLOCK)-sizeof(PLOBHEADER))*nBitsPerByte,
	0,
	(LPCSTR) NULL,
        (TYPEFLAGS) ( (unsigned int) typeRecycleP |
		      (unsigned int) typeNotCachableP ) },
    { (SHTYPETAG) eshOneLockTag,
        SZPLOB,
	"one-lock",
	(sizeof(ONELOCK)-sizeof(PLOBHEADER))*nBitsPerByte,
	0,
	(LPCSTR) NULL,
        (TYPEFLAGS) ( (unsigned int) typeRecycleP |
		      (unsigned int) typeNotCachableP ) },
    { (SHTYPETAG) eshQueueEntryTag,
        SZPLOB,
	"suspended-queue-entry",
	(sizeof(QUEUENTRY)-sizeof(PLOBHEADER))*nBitsPerByte,
	0,
	(LPCSTR) NULL,
        (TYPEFLAGS) ( (unsigned int) typeRecycleP |
		      (unsigned int) typeNotCachableP ) }
  ...
  PROCEDURE	( fnInitCommonLockModule );
...
{
  PROCEDURE	( fnDeinitCommonLockModule );
