...
(defconstant +plob-dir+
    #+:linux "/home/heiko/heiko-ws/plob-2.11"
    #+:solaris2 "/home/kirschke/plob-2.11"
    #+(or :win32 :mswindows) ;; "h:/plob-2.11"
    "E:/home/heiko/plob-2.11"
...
(defparameter +plob-lisp-directory+
    ...
    ;;; 2005-04-14 hkirschk: Now one common source directory for both
...
(defparameter +plob-target-lisp-directory+
  (or
   ...
   #+(and :allegro (version>= 5)) "/allegro5"
   #+(and :allegro (version>= 4)) "/allegro4"
   #+lispworks4.4 "/lispworks44"
   #+lispworks4.3 "/lispworks43"
   #+lispworks4.2 "/lispworks42"
   #+lispworks4.1 "/lispworks41"
   #+lispworks3 "/lispworks3"
   "")
    "
\\Purposelabel
...
(defun namestring-if (pathname)
  (namestring (enough-namestring 
               (if (eq (class-of pathname) +logical-pathname-class+)
                   (translate-logical-pathname pathname)
                 pathname)
               (concatenate 'string +plob-dir+ "/src/"))))

;;; --------------------------------------------------------------------------
;;; 2005-04-14 hkirschk: :small-fixnum on *features* means that a native
;;; fixnum is too small to capture an objid; an integer has to be used
;;; instead
#+:lispworks
(pushnew :small-fixnum *features*)

;;; --------------------------------------------------------------------------
#+:lispworks
(eval `(defsystem::defsystem :plob
	   (:default-pathname ,(concatenate 'string +plob-dir+ "/src/")
	    ;;; 2005-04-14 hkirschk: Hmmm, looks as if
            ;;; :object-pathname does not work at all ...
            :object-pathname ,(concatenate 'string +plob-dir+ "/src/"
                                           +plob-lisp-directory+
                                           +plob-target-lisp-directory+))
