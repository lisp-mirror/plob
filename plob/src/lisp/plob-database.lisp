...
\\Seealsolabel
 ...
 \\fcite{mapbtree}; section \\fcite{index ...}."
  ...
  (when (and like not-like)
    (error "Only none or eaxactly one of :like or :not-like may be given."))
  (let ((index (get-index-table the-class slot-name)))
	...
	(let ((argument-list
	       `(:key-depth :objid :data-depth :objid
			    :p-heap ,p-heap)))
	  (when (or like not-like)
	    (setf filter (p-make-regex (if like like not-like)
				       :notmatching not-like))
	    (setf *filter* filter)
	    (push filter argument-list)
	    (push :filter argument-list))
	  (when lessp
      ...
      (when filter
	;; destroy the regex pattern matcher after usage:
	;; (p-destroy filter)
	)
      selected)))
