...
;;; ---------------------------------------------------------------------------
(defconstant +plob-match-any-marker+
    (make-persistent-object +match-any-tag+ +match-any-tag+)
  #+:lisp-doc "Object used for representing a match any object.
A comparision with this object always means that both objects are equal.")

;;; ---------------------------------------------------------------------------
(defconstant +plob-match-never-marker+
    (make-persistent-object +match-never-tag+ +match-never-tag+)
  #+:lisp-doc "Object used for representing a match any object.
A comparision with this object always means that both objects are not equal.")
...
;;; ---------------------------------------------------------------------------
(defmethod p-objid-to-t-object (p-objid
				(p-objid-type-tag (eql +min-tag+))
				depth p-heap)
  #+:lisp-doc "Returns the \\fcite{+plob-min-marker+}."
  (declare (ignore p-objid depth p-heap))
  +plob-min-marker+)

;;; ---------------------------------------------------------------------------
(defmethod p-objid-to-t-object (p-objid
				(p-objid-type-tag (eql +max-tag+))
				depth p-heap)
  #+:lisp-doc "Returns the \\fcite{+plob-max-marker+}."
  (declare (ignore p-objid depth p-heap))
  +plob-max-marker+)

;;; ---------------------------------------------------------------------------
(defmethod p-objid-to-t-object (p-objid
				(p-objid-type-tag (eql +match-any-tag+))
				depth p-heap)
  #+:lisp-doc "Returns the \\fcite{+plob-match-any-marker+}."
  (declare (ignore p-objid depth p-heap))
  +plob-match-any-marker+)

;;; ---------------------------------------------------------------------------
(defmethod p-objid-to-t-object (p-objid
				(p-objid-type-tag (eql +match-never-tag+))
				depth p-heap)
  #+:lisp-doc "Returns the \\fcite{+plob-match-never-marker+}."
  (declare (ignore p-objid depth p-heap))
  +plob-match-never-marker+)
