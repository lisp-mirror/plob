...
(defun sh-create-object (p-heap-objid type-tag
  ...
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
		 ...
		 #+:small-fixnum integer
		 p-heap-objid)
	   (type fixnum type-tag number-of-extra-objids number-of-extra-values))
...
(defun sh-write-instance-data (p-heap-objid p-objid-instance p-objid-data)
 ...
 \\funarg{p-objid-instance}."
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
	         #-:small-fixnum fixnum
		 #+:small-fixnum integer
		 p-heap-objid p-objid-instance p-objid-data))
...
;;; ---------------------------------------------------------------------------
(defun sh-exit (p-heap-objid &optional force)
  #+:lisp-doc "
\\Purposelabel
 Exit the \\plob\\ daemon server process."
  ...
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
		 ...
		 #+:small-fixnum integer
		 p-heap-objid))
  (/= (catch-errors (c-sh-exit p-heap-objid (if force +c-true+ +c-false+)))
      +c-false+))
  ...
  (let* ((eff-url (effective-url url))
    ...
    (sh-load (if (equal transport "local")
		 +lib-local-client-plob+
	       +lib-rpc-client-plob+))
    (let* ((done (c-sh-connect (write-url eff-url)))
	   (p-heap-objid
	    (when (/= done +c-false+)
	      (catch-errors
	       (c-sh-open description
			  (if *max-lisp-addr*
			      (floor *max-lisp-addr* 1024)
			    0))))))
      (if (or (null p-heap-objid) (= p-heap-objid +null-objid+))
...
\\Seealsolabel
 ...
 \\fcite{sh-write-double-float}."
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
	         #-:small-fixnum fixnum
		 #+:small-fixnum integer
		 p-heap-objid) (type double-float from))
...
\\Remarkslabel
 ...
 an active transaction on \\funarg{p-heap-objid}."
  ...
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
		 ...
		 #+:small-fixnum integer
		 p-heap-objid p-objid)
	   (type fixnum start-index number-of-objids))
  (with-handle-lock-conflict
   #'(lambda (p-heap-objid p-objid)
       (catch-errors
...
\\Seealsolabel
 ...
 \\shcite{function}{SH\\us{}read\\us{}words}{5}."
  ...
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
		 ...
		 #+:small-fixnum integer
		 p-heap-objid p-objid)
	   (type fixnum number-of-characters))
  (with-handle-lock-conflict
   #'(lambda (p-heap-objid p-objid)
       (catch-errors
...
\\Seealsolabel
 ...
 \\shcite{function}{SH\\us{}read\\us{}words}{5}."
  ...
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
		 ...
		 #+:small-fixnum integer
		 p-heap-objid p-objid)
	   (type fixnum number-of-elements))
  (with-handle-lock-conflict
   #'(lambda (p-heap-objid p-objid)
       (catch-errors
...
;;; ---------------------------------------------------------------------------
(defun sh-reset (p-heap-objid &optional force)
...
\\Purposelabel
 Reset the \\plob\\ daemon server process."
  ...
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
		 ...
		 #+:small-fixnum integer
		 p-heap-objid))
  (/= (catch-errors (c-sh-reset p-heap-objid (if force +c-true+ +c-false+)))
      +c-false+))

;;; ---------------------------------------------------------------------------
(defun sh-restart (p-heap-objid &optional force)
  #+:lisp-doc "
\\Purposelabel
 Restart the \\plob\\ daemon server process."
  ...
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
		 ...
		 #+:small-fixnum integer
		 p-heap-objid))
  (/= (catch-errors (c-sh-restart p-heap-objid (if force +c-true+ +c-false+)))
      +c-false+))

;;; ---------------------------------------------------------------------------
(defun sh-suspend (p-heap-objid &optional description)
  #+:lisp-doc "
\\Purposelabel
 Suspend the \\plob\\ daemon server process."
  ...
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
		 ...
		 #+:small-fixnum integer
		 p-heap-objid))
  (catch-errors (c-sh-suspend p-heap-objid (if description description ""))))
...
\\Seealsolabel
 ...
 \\fcite{sh-read-fixnum}."
  ...
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
		 ...
		 #+:small-fixnum integer
		 p-heap-objid p-objid)
	   (type fixnum at-index the-fixnum))
  (with-handle-lock-conflict
   #'(lambda (p-heap-objid p-objid)
       (catch-errors
...
\\Seealsolabel
 ...
 \\fcite{sh-read-index}."
  ...
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
	         #-:small-fixnum fixnum
		 #+:small-fixnum integer
		 p-heap-objid p-objid immediate-value)
	   (type fixnum at-index immediate-type-tag))
  (with-handle-lock-conflict
   #'(lambda (p-heap-objid p-objid)
       (catch-errors
...
\\Seealsolabel
 ...
 \\fcite{sh-read-objid}."
  ...
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
	         #-:small-fixnum fixnum
		 #+:small-fixnum integer
		 p-heap-objid p-objid the-p-objid)
	   (type fixnum at-index))
  (with-handle-lock-conflict
   #'(lambda (p-heap-objid p-objid)
       (catch-errors
...
\\Seealsolabel
 ...
 \\fcite{sh-insert-lock}."
  ...
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
		 ...
		 #+:small-fixnum integer
		 p-heap-objid p-objid)
	   (type fixnum lock-mode at-index))
  (catch-errors
...
\\Seealsolabel
 \\Fcite{rembtree}."
  ...
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
	         #-:small-fixnum fixnum
		 #+:small-fixnum integer
		 p-objid-btree immediate-key-value)
	   (type fixnum immediate-key-type-tag))
...
\\Seealsolabel
 \\Fcite{(setf getbtree-with-data)}."
  ...
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
	         #-:small-fixnum fixnum
		 #+:small-fixnum integer
		 p-objid-btree immediate-key-value immediate-data-value)
	   (type fixnum immediate-key-type-tag immediate-data-type-tag))
...
\\Seealsolabel
 \\Fcite{(setf getbtree-with-data) (t float t t t t t)}."
  ...
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
	         #-:small-fixnum fixnum
		 #+:small-fixnum integer
		 p-objid-btree immediate-data-value)
	   (type fixnum immediate-data-type-tag))
  (sh-btree-insert-return-value
   (with-handle-lock-conflict
    #'(lambda (p-heap-objid p-objid-btree)
	(catch-errors
...
(defun sh-btree-map-search-by-float
  (p-heap-objid p-objid-mapper p-objid-btree
		...
		(immediate-key-end-type-tag +max-tag+)
		(compare-end-key +less-equal+)
		descending (p-objid-filter +null-objid+))
  (with-handle-lock-conflict
   #'(lambda (p-heap-objid p-objid-mapper)
       (catch-errors
...
\\Seealsolabel
 ...
 \\fcite{mapbtree-internal (t t persistent-object t t t t)}."
  ...
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
	         #-:small-fixnum fixnum
		 #+:small-fixnum integer
		 p-heap-objid p-objid-btree p-objid-filter
		 immediate-key-start-value immediate-key-end-value)
	   (type fixnum immediate-key-start-type-tag immediate-key-end-type-tag))
    ...
    (loop as iterated of-type fixnum = 0
      ...
      for mapped = (multiple-value-setq
			       ...
			       compare-start-key
                               immediate-key-end-value
                               immediate-key-end-type-tag
			       compare-end-key descending p-objid-filter
                               +map-buffer-size+
...
;;; ---------------------------------------------------------------------------
(defun map-comparetag-to-keyword (comparetag)
  (declare (inline map-comparetag-to-keyword))
  (ecase comparetag
     (#.+equal+		:equal)
     (#.+eql+		:eql)
     (#.+eq+		:eq)
...
;;; ---------------------------------------------------------------------------
(defun sh-compare (p-heap-objid
		   immediate-value-1 immediate-type-tag-1
		   immediate-value-2 immediate-type-tag-2)
  (declare (type ;;; 2005-04-06 hkirschk: Corrected for LispWorks 4.4
	    #-:small-fixnum fixnum
	    #+:small-fixnum integer
	    p-heap-objid immediate-value-1 immediate-value-2)
	   (type fixnum immediate-type-tag-1 immediate-type-tag-2))
  (catch-errors
   (map-comparetag-to-keyword
       (c-sh-compare p-heap-objid
		     immediate-value-1 immediate-type-tag-1
		     immediate-value-2 immediate-type-tag-2))))

;;; ---------------------------------------------------------------------------
(defun sh-make-regex (p-heap-objid pattern regcomp-flags regexec-flags)
  (declare
   (type string pattern)
   (type fixnum regcomp-flags regexec-flags))
  (catch-errors
   (c-sh-make-regex p-heap-objid pattern regcomp-flags regexec-flags)))

;;; ---------------------------------------------------------------------------
(defun sh-compile-regex (p-heap-objid p-regex-objid)
  (declare (type
	    #-:small-fixnum fixnum
	    #+:small-fixnum integer
	    p-heap-objid p-regex-objid))
  (let* ((buffer (make-string +pprint-buffer-size+
			      :initial-element #\Space))
         (error (catch-errors
                  (c-sh-compile-regex p-heap-objid p-regex-objid
                                      buffer +pprint-buffer-size+))))
    (when (> error 0)
      (values error buffer))))
