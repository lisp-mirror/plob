...
static psint	FLsize,FLmax,*reuseList,*freeList ;
...
static void reset_freelist()				/* clear freeList entries for pages on the reuseList */
{
	psint cnt,*tptr,*tptr2 ;
...
static void setup_freelist()
{
	...
	FLmax = ( VASPACEPAGES << 1 ) & ~31 ;			/* Count number of va pages - double and round down to a mult of 32 */

	freeList = ( psint * ) memalign( WORDSIZE,FLmax >> 3 ) ;		/* create a bitmap */
	if ( freeList == ( psint * ) 0 )
		error( "failed to initialise the store file freelist" ) ;

	reuseList = ( psint * ) memalign( WORDSIZE,FLmax >> 3 ) ;		/* create a bitmap */
...
/* Given a page find its blockNo */
#define getPteBlockNo( p ) ( *getPteAddr( p ) & LOWER24 )
...
psint	p,protection ;
{
	...
	if ( blockNo == FLsize )		/* No free disk blocks - extend file */
	{
		...
		if ( FLsize == FLmax )
			return( PSFALSE ) ;		/* Store file is max size to support the max VA size */
...
psint	p ;						/* and read protect the page */
{
	int m_res ;
	caddr_t start ;

	start = ( caddr_t )( ( char * ) disk_vaddr + p ) ;
	m_res = msync( start,BPAGESIZE,0 ) ;
	if ( m_res != 0 )
		error( "failed to synchronise a page with the store file" ) ;
	m_res = mprotect( start,BPAGESIZE,PROT_READ|PROT_EXEC ) ;
	if ( m_res != 0 )
...
{
	psint 	i,j,blockNo,flags,*pte,p,q ;
			...
			if ( flags & WRITTEN )			/* Shadowed or SaveOnly */
			{
				...
				for ( j = ( psint ) 0 ; j < WPAGESIZE ; j++ )
				{
					...
					if ( pte[ j ]  != ( psint ) 0 )
					{
						...
						{
							sync_page( q ) ;
							setPteFlags( q,RO ) ;		/* page is now readonly */
						}
					}
					q += BPAGESIZE ;
				}
				sync_page( p ) ;
				setPteFlags( p,RO ) ; 			/* page is now readonly */
			}
		}
	}
}
...
char *dirname ;
{
	...
	remap_pagetables() ;			/* shadow copy the pagetables */
	...
	signal( SIGBUS,page_fault ) ;		/* enable the page_fault handler - to cope with a pre4.1.2 bug */
...
psint page ;
{
	...
	if ( result )
	{
		...
		RLClear( getPteBlockNo( page ) ) ;	/* mark the old block as free from the net checkpoint */
...
psint a1,a2 ;
{
	psint pte,flags,t,p1,p2,pp1,pp2 ;
	...
	p1 = a1 & PAGENUMBER ;			/* make sure we've remembered the two page numbers */
	p2 = a2 & PAGENUMBER ;
